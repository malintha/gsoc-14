  ...
  var organizer = {
      getctag : 1378022830,
        ...
        icalString :      'BEGIN:VEVENT\n' + 
                          ...
                          'LOCATION:Paris\n'+
                          'TRANSP:OPAQUE\n'+
                          'ORGANIZER;CN=Organizer Name;SENT-BY="mailto:organizer@example.com":mailto:organizer@example.com\n'+
        ...
        userAddressSet : ["organizer@example.com"],
...
// schedule-tag and etag of attendees' object resource
  var attendee1 = {
        ...
        supportedComps : ["VEVENT","VTODO"],
        userAddressSet : ["attendee1@example.com"],
        getetag : 1114233447,
        scheduletag : 23456
  };
  ...
  function run_test() {
    ...
    // attendee1's handlers registration
    ...
    //organizer's handlers registration
...
/**
 *  Event is in the organizers calendar and attendee1's calendars. Organizer does a change to the event now.
 *  Since the change is not a PARTSTAT change, schedule tags should be changed and the change should be merged 
 *  with attendee1's calendar.
 *
 */   
...
// retrieve the item on behalf of the organizer considering the schedule-tag
...
// check modified item properties
  ...
  /**
   *  now event should retrieve from attendee1's calendar and assert it against
   *  changed event of organizer's event
   */
  ...
  try {
    ...
    if(method=="PUT" && matchheader=="*" && body){
      ...
      dump("got scheduletag:");
        // update object resources. here organizer's and attendees resources
    // should be changed and schedule-tags as well.
